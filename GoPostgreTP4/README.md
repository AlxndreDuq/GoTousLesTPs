# TP #4 - PostgreSQL, enrichissement automatique et recherche avancée

Trois évolutions majeures de `mira` :

**1. Brancher PostgreSQL** (fini le stockage en mémoire) :

- schéma SQL + migrations (`notes`, `note_tags`, `note_embeddings`)
- repository PG via **pgx**
- transaction sur création de note + tags

**2. Enrichissement automatique** — l'enrichissement se déclenche **au fil de l'eau** :

- à chaque `POST` ou `PATCH` sur une note, un job d'enrichissement est publié dans un **channel interne**
- un **pool de workers** borné consomme les jobs et applique les enrichissements (tags, résumé court, score, embeddings)
- chaque tâche a un **timeout via context**
- le résultat est écrit en base ; la note porte un statut `enrichment_status` (`pending` / `done` / `failed`)
- l'écriture de la note reste **synchrone et rapide** : l'API répond immédiatement, l'enrichissement suit en asynchrone

**3. Recherche avancée** :

- recherche hybride : full-text + similarité vectorielle (sur les embeddings produits par l'enrichissement)
- index utiles créés par migration (GIN full-text + vector index)

> ⚠️ **La CLI passe désormais par l'API** (client HTTP), et non plus par le fichier JSONL local :
> c'est le seul moyen de garantir que chaque note créée ou modifiée déclenche l'enrichissement automatique.
